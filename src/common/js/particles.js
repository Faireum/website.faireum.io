  ...
  window.particles = function($id, $color, $count) {
  /*
  ...
  (function(w) {
    ...
    */
    ...
    if(w.innerWidth && w.innerWidth != 0) {
      canvas_width = Math.ceil(w.innerWidth / resolution) * resolution
    }
    if(w.innerHeight && w.innerHeight != 0) {
      canvas_height = Math.ceil(w.innerHeight / resolution) * resolution
    }
    
    var pen_size = 150; //Radius around the mouse cursor coordinates to reach when stirring
    ...
    var speck_count = $count; //This determines how many particles will be made.
    ...
    */
    ...
    function autoPlay() {
        mouse.px = mouse.x;
        mouse.py = mouse.y;
        mouse.x = mouse.x + (Math.random() * canvas_width - mouse.x)/30;
        mouse.y = mouse.y + (Math.random() * canvas_height - mouse.y)/30;
    }
